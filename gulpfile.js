...
gulp.task('img-resize', function () {
  return gulp.src('src/*.{jpg,png}').pipe(gResponsive({
    /*
    '*.jpg': {
    ...
    */
    ...
    quality: 100, // default 80. The output quality for JPEG, WebP and TIFF
                  // output formats
    progressive: true, // Use progressive (interlace) scan for JPEG and PNG
                       // output
...
gulp.task('rename-img-add-index', function () {
  ...
  return gulp.src('src/*.{jpg,JPG}').pipe(rename(function (path) {
...
gulp.task('img-resize-square', function () {
  // case sensitive
  return gulp.src('src/*.JPG').pipe(gResponsive({
    '*': {
      width: 140,
      height: 140,
...
gulp.task('img-resize-max-width-sharp', function () {
  ...
  return gulp.src('src/**/*').pipe(gResponsive({
    ...
    errorOnUnusedConfig: false, // don't emit error when a filter is not used
    errorOnUnusedImage: false, // don't emit error when an image is not used
    errorOnEnlargement: false, // skip error when final image is enlarged or
                              // shrunk
  })).pipe(gulp.dest('dist'))
})
...
gulp.task('img-convert-webp', function () {
  // case sensitive
  return gulp.src('src/*.png').pipe(gResponsive({
    '*': {
...
gulp.task('img-min', () => {
  return gulp.src('src/**/*').pipe(gPlumber()).pipe(gSort()).pipe(gImageMin(
    // [plugins], default is
...
gulp.task('img-min-parallel', () => {
  var threads = 8
  return gulp.src('src/**/*').pipe(gPlumber()).pipe(gSort()).pipe(
    gParallel(
      gImageMin(
        [
          gImageMin.gifsicle({ interlaced: true }),
          ...
          gImageMin.svgo(), // default will move <style> to attributes and
                           // remove svg[id]
        ]
        , {
          verbose: true, // default false. Log for every image passed
...
gulp.task('img-min-rename', () => {
  return gulp.src('src/**/*').pipe(gImageMin(
    // [plugins], default is
    //[imagemin.gifsicle(), imagemin.jpegtran(), imagemin.optipng(),
    // imagemin.svgo()]
    ...
    */
    mode: {
      symbol: { // symbol mode to build the SVG
        // extra css/scss files are not needed because the example html for
        // symbol mode is enough
        render: {
          css: false, // false to not generate CSS output option for icon
                      // sizing. {example:true}.
          scss: false, // SCSS output option for icon sizing
        },
        dest: 'sprite', // destination folder combined with gulp.dest e.g.
                        // dist/sprite
        prefix: '.svg--%s', // BEM-style prefix if styles rendered. See
                            // dimensions
        // dimensions: '-dims', // combine with prefix, the css class is
        // .svg--plus-dims. So that you can inline svg
        /*
  ...
  var config = {
      ...
      transform: [
        {
          svgo: {
            plugins: [
              //{convertStyleToAttrs: true} // enable a plugin
              { inlineStyles: { onlyMatchedOnce: false } }, // Since HTML5 has
                                                           // <style> stripped
                                                           // when svg file is
                                                           // not inline with
                                                           // HTML, we need to
    ...
    mode: {
      symbol: { // symbol mode to build the SVG
        ...
        //bust:true, // add hash to sprite.svg e.g. sprite-88ac2671.svg
        example: { dest: 'sprite-css-inline.html' }, // Build a sample page,
                                                    // please! e.g.
                                                    // dist/sprite/sprite.symbol.html
      },
    },
  }
...
gulp.task('svg-sprite-no-style-tag', function () {
  var config = {
    shape: {
      id: { generator: 'icon-%s' }, // change shape ID's. Default shape ID is
                                    // individual svg file name
      transform: [
        {
          svgo: {
            plugins: [
              { removeXMLProcInst: true }, // it seems xml header is not
                                           // removed..
    ...
    mode: {
      symbol: { // symbol mode to build the SVG
        ...
        //bust:true, // add hash to sprite.svg e.g. sprite-88ac2671.svg
        example: { dest: 'sprite-no-style-tag.html' }, // Build a sample page,
                                                      // please! e.g.
                                                      // dist/sprite/sprite.symbol.html
      },
    },
  ...
  var config = {
    ...
    mode: {
      view: { // symbol mode to build the SVG
        ...
        //bust:true, // add hash to sprite.svg e.g. sprite-88ac2671.svg
        example: { dest: 'sprite-mode-view.html' }, // Build a sample page,
                                                   // please! e.g.
                                                   // dist/sprite/sprite.symbol.html
      },
    },
